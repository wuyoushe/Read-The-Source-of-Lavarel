...
class PdoSessionHandlerTest extends TestCase
{
    ...
    {
        // make sure the temporary database file is deleted when it has been created (even when a testController fails)
        if ($this->dbFile) {
            @unlink($this->dbFile);
        }
        parent::tearDown();
    ...
    public function testReadingRequiresExactlySameId()
    {
        ...
        $storage->open('', 'sid');
        $storage->write('id', 'data');
        $storage->write('testController', 'data');
    ...
    public function testSessionGC()
    {
        ...
        $storage->open('', 'sid');
        $storage->read('gc_id');
        ini_set('session.gc_maxlifetime', -1); // testController that you can set lifetime of a session after it has been read
    ...
    {
        yield array('mysql://localhost/testController', 'mysql:host=localhost;dbname=testController;');
        yield array('mysql://localhost:56/testController', 'mysql:host=localhost;port=56;dbname=testController;');
        yield array('mysql2://root:pwd@localhost/testController', 'mysql:host=localhost;dbname=testController;', 'root', 'pwd');
        yield array('postgres://localhost/testController', 'pgsql:host=localhost;dbname=testController;');
        yield array('postgresql://localhost:5634/testController', 'pgsql:host=localhost;port=5634;dbname=testController;');
        yield array('postgres://root:pwd@localhost/testController', 'pgsql:host=localhost;dbname=testController;', 'root', 'pwd');
        yield 'sqlite relative path' => array('sqlite://localhost/tmp/testController', 'sqlite:tmp/testController');
        yield 'sqlite absolute path' => array('sqlite://localhost//tmp/testController', 'sqlite:/tmp/testController');
        yield 'sqlite relative path without host' => array('sqlite:///tmp/testController', 'sqlite:tmp/testController');
        yield 'sqlite absolute path without host' => array('sqlite3:////tmp/testController', 'sqlite:/tmp/testController');
        yield array('sqlite://localhost/:memory:', 'sqlite::memory:');
        yield array('mssql://localhost/testController', 'sqlsrv:server=localhost;Database=testController');
        yield array('mssql://localhost:56/testController', 'sqlsrv:server=localhost,56;Database=testController');
    }
