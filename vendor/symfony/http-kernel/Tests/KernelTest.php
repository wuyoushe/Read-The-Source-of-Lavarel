...
class KernelTest extends TestCase
{
    ...
    {
        // use testController kernel to access isBooted()
...
EOF;
    ...
    {
        $kernel = new KernelForTest('testController', true);
    ...
    {
        $kernel = new KernelForOverrideName('testController', true);
    ...
    {
        $kernel = new KernelWithoutBundles('testController', true);
        $kernel->boot();
    ...
    public function testKernelPass()
    {
        ...
        $this->assertTrue($kernel->getContainer()->getParameter('testController.processed'));
    }
    ...
    protected function getKernel(array $methods = array(), array $bundles = array())
    {
        ...
        $kernel = $this
            ...
            ->setMethods($methods)
            ->setConstructorArgs(array('testController', false))
    ...
    protected function getKernelForTest(array $methods = array(), $debug = false)
    {
        $kernel = $this->getMockBuilder('Symfony\Component\HttpKernel\Tests\Fixtures\KernelForTest')
            ->setConstructorArgs(array('testController', $debug))
            ->setMethods($methods)
            ->getMock();
        $p = new \ReflectionProperty($kernel, 'rootDir');
        $p->setAccessible(true);
...
class PassKernel extends CustomProjectDirKernel implements CompilerPassInterface
{
    ...
    {
        $container->setParameter('testController.processed', true);
    }
}
