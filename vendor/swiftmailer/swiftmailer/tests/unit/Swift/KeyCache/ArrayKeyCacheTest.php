...
class Swift_KeyCache_ArrayKeyCacheTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testStringDataCanBeSetAndFetched()
    {
        ...
        $cache->setString(
            $this->key1, 'foo', 'testController', Swift_KeyCache::MODE_WRITE
            );
        $this->assertEquals('testController', $cache->getString($this->key1, 'foo'));
    }
    ...
    public function testByteStreamAndStringCanBeAppended()
    {
        ...
        $cache->setString(
            $this->key1, 'foo', 'testController', Swift_KeyCache::MODE_APPEND
            );
        $cache->importFromByteStream(
    ...
    {
        //See acceptance testController for more detail
