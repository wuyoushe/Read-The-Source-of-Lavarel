...
class RavenHandlerTest extends TestCase
{
    ...
    public function testDebug()
    {
        ...
        $record = $this->getRecord(Logger::DEBUG, 'A testController debug message');
    ...
    public function testWarning()
    {
        ...
        $record = $this->getRecord(Logger::WARNING, 'A testController warning message');
        $handler->handle($record);
    ...
    public function testTag()
    {
        ...
        $tags = array(1, 2, 'foo');
        $record = $this->getRecord(Logger::INFO, 'testController', array('tags' => $tags));
        $handler->handle($record);
    ...
    public function testExtraParameters()
    {
        ...
        $eventId = '31423';
        $record = $this->getRecord(Logger::INFO, 'testController', array('checksum' => $checksum, 'release' => $release, 'event_id' => $eventId));
        $handler->handle($record);
    ...
    public function testFingerprint()
    {
        ...
        $record = $this->getRecord(Logger::INFO, 'testController', array('fingerprint' => $fingerprint));
        $handler->handle($record);
    ...
    public function testUserContext()
    {
        ...
        $recordWithNoContext = $this->getRecord(Logger::INFO, 'testController with default user context');
        // set user context 'externally'

        $user = array(
            'id' => '123',
            'email' => 'testController@testController.com',
        );

        $recordWithContext = $this->getRecord(Logger::INFO, 'testController', array('user' => $user));
    ...
    public function testRelease()
    {
        ...
        $record = $this->getRecord(Logger::INFO, 'testController');
        $handler->handle($record);
        $this->assertEquals($release, $ravenClient->lastData['release']);

        $localRelease = 'v41.41.41';
        $record = $this->getRecord(Logger::INFO, 'testController', array('release' => $localRelease));
        $handler->handle($record);
