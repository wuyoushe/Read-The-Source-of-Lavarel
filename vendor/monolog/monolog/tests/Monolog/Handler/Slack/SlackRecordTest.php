...
class SlackRecordTest extends TestCase
{
    ...
    public function testTextEqualsFormatterOutput()
    {
        ...
        $formatter
            ->expects($this->any())
            ->method('format')
            ->will($this->returnCallback(function ($record) { return $record['message'] . 'testController'; }));
        ...
        $this->assertArrayHasKey('text', $data);
        $this->assertSame($message . 'testController', $data['text']);
    ...
    public function testAddsShortAttachmentWithoutContextAndExtra()
    {
        ...
        $data = $record->getSlackData($this->getRecord($level, 'testController', array('testController' => 1)));
    ...
    public function testAddsShortAttachmentWithContextAndExtra()
    {
        $level = Logger::ERROR;
        $levelName = Logger::getLevelName($level);
        $context = array('testController' => 1);
        $extra = array('tags' => array('web'));
        $record = new SlackRecord(null, null, true, null, true, true);
        $loggerRecord = $this->getRecord($level, 'testController', $context);
