...
class TestHandlerTest extends TestCase
{
    ...
    public function testHandler($method, $level)
    {
        ...
        $record = $this->getRecord($level, 'testController' .$method);
        ...
        $this->assertFalse($handler->{'has'.$method.'ThatContains'}('testController'), 'has'.$method.'ThatContains');
        $this->assertFalse($handler->{'has'.$method.'ThatPasses'}(function ($rec) {
            return true;
        }), 'has'.$method.'ThatPasses');
        $this->assertFalse($handler->{'has'.$method.'ThatMatches'}('/testController\w+/'));
        ...
        $this->assertTrue($handler->{'has'.$method}('testController' .$method), 'has'.$method);
        $this->assertTrue($handler->{'has'.$method.'ThatContains'}('testController'), 'has'.$method.'ThatContains');
        $this->assertTrue($handler->{'has'.$method.'ThatPasses'}(function ($rec) {
            return true;
        }), 'has'.$method.'ThatPasses');
        $this->assertTrue($handler->{'has'.$method.'ThatMatches'}('/testController\w+/'));
