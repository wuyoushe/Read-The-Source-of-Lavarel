...
class FlowdockFormatterTest extends TestCase
{
    ...
    {
        $formatter = new FlowdockFormatter('test_source', 'source@testController.com');
        $record = $this->getRecord();

        $expected = array(
            'source' => 'test_source',
            'from_address' => 'source@testController.com',
            'subject' => 'in test_source: WARNING - testController',
            'content' => 'testController',
            'tags' => array('#logs', '#warning', '#testController'),
            'project' => 'test_source',
        );
