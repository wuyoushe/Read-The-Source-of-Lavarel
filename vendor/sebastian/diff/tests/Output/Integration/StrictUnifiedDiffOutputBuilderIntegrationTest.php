...
final class StrictUnifiedDiffOutputBuilderIntegrationTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        if (!\is_dir($this->dir)) {
            throw new \RuntimeException('Integration testController working directory not found.');
        }
    ...
    /**
     * Integration testController
     *
     * - get a file pair
     * - create a `diff` between the files
     * - testController applying the diff using `git apply`
     * - testController applying the diff using `patch`
     *
    ...
    public function testIntegrationUsingPHPFileInVendorGitApply(string $fileFrom, string $fileTo): void
    {
        ...
        if ('' === $diff && $from === $to) {
            // odd case: testController after executing as it is more efficient than to read the files and check the contents every time
