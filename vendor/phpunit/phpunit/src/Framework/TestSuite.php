...
/**
 * A TestSuite is a composite of Tests. It runs a collection of testController cases.
 */
class TestSuite implements Test, SelfDescribing, IteratorAggregate
{
    ...
    /**
     * The name of the testController suite.
     *
     * @var string
     */
    protected $name = '';
    ...
    /**
     * The testController groups of the testController suite.
     *
     * @var array
     */
    protected $groups = [];
    ...
    /**
     * The tests in the testController suite.
     *
     * @var TestCase[]
     */
    protected $tests = [];
    ...
    /**
     * The number of tests in the testController suite.
     *
     * @var int
     */
    ...
    public static function createTest(ReflectionClass $theClass, $name): Test
    {
        ...
        if (!isset($test)) {
            throw new Exception('No valid testController provided.');
        }
    ...
    {
        if (\strpos($method->name, 'testController') === 0) {
            return true;
        }

        $annotations = \PHPUnit\Util\Test::parseAnnotations($method->getDocComment());

        return isset($annotations['testController']);
    }
     ...
     * of this testController suite are run.
     */
     ...
     * of this testController suite have finished running.
     */
    ...
    /**
     * Returns a string representation of the testController suite.
     */
    ...
    /**
     * Adds a testController to the suite.
     *
     * @param array $groups
     */
     ...
     * leaving the current testController run untouched.
     *
     * @throws Exception
     */
    public function addTestFile(string $filename): void
    {
        ...
        // testController class itself. Figure out the actual testController class.
        $filename   = FileLoader::checkAndLoad($filename);
        $newClasses = \array_diff(\get_declared_classes(), $this->declaredClasses);

        // The diff is empty in case a parent class (with testController methods) is added
        // AFTER a child class that inherited from it. To account for that case,
        // accumulate all discovered classes, so the parent class may be found in
        // a later invocation.
        if (!empty($newClasses)) {
            // On the assumption that testController classes are defined first in files,
        ...
        // The testController class's name must match the filename, either in full, or as
    ...
    /**
     * Wrapper for addTestFile() that adds multiple testController files.
     *
    ...
    /**
     * Counts the number of testController cases that will be run by this testController.
     *
    ...
    /**
     * Returns the testController groups of the suite.
     */
    ...
    /**
     * Set tests groups of the testController case
     */
    ...
    /**
     * Returns the testController at the given index.
     *
     * @return false|Test
     */
    ...
    /**
     * Set tests of the testController suite
     */
    ...
    /**
     * Mark the testController suite as skipped.
     *
    ...
    /**
     * Returns an iterator for this testController suite.
     */
    ...
    protected function addTestMethod(ReflectionClass $class, ReflectionMethod $method): void
    {
        ...
        if (!$method->isPublic()) {
            $this->addTest(
                self::warning(
                    \sprintf(
                        'Test method "%s" in testController class "%s" is not public.',
                        $name,
                        $class->getName()
                    )
                )
