...
class ResultPrinter extends Printer implements TestListener
{
    ...
    {
        $this->printDefects($result->notImplemented(), 'incomplete testController');
    }

    protected function printRisky(TestResult $result): void
    {
        $this->printDefects($result->risky(), 'risky testController');
    }

    protected function printSkipped(TestResult $result): void
    {
        $this->printDefects($result->skipped(), 'skipped testController');
    }
    ...
    protected function printFooter(TestResult $result): void
    {
        ...
        if ($result->wasSuccessful() &&
            ...
            $this->writeWithColor(
                'fg-black, bg-green',
                \sprintf(
                    'OK (%d testController%s, %d assertion%s)',
                    \count($result),
                    (\count($result) == 1) ? '' : 's',
                    $this->numAssertions,
