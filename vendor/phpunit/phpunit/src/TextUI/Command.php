...
class Command
{
    ...
    protected $longOptions = [
        ...
        'coverage-xml='             => null,
        'debug'                     => null,
        'disallow-testController-output'      => null,
    ...
    public function run(array $argv, bool $exit = true): int
    {
        ...
        if ($this->arguments['testController'] instanceof Test) {
            $suite = $this->arguments['testController'];
        } else {
            $suite = $runner->getTest(
                $this->arguments['testController'],
        ...
        unset($this->arguments['testController'], $this->arguments['testFile']
        );
    ...
    protected function handleArguments(array $argv): void
    {
        ...
        foreach ($this->options[0] as $option) {
            switch ($option[0]) {
                ...
                case '--disallow-testController-output':
        ...
        if (!isset($this->arguments['testController'])) {
            if (isset($this->options[1][0])) {
                $this->arguments['testController'] = $this->options[1][0];
            }
            ...
            if (isset($this->arguments['testController']) &&
                \is_file($this->arguments['testController']) &&
                \substr($this->arguments['testController'], -5, 5) != '.phpt') {
                $this->arguments['testFile'] = \realpath($this->arguments['testController']);
                $this->arguments['testController']     = \substr($this->arguments['testController'], 0, \strrpos($this->arguments['testController'], '.'));
            }
        }
        ...
        if (isset($this->arguments['configuration'])) {
            ...
            if (!isset($this->arguments['testController'])) {
                ...
                if ($testSuite !== null) {
                    $this->arguments['testController'] = $testSuite;
                }
            }
        ...
        if (isset($this->arguments['testController']) && \is_string($this->arguments['testController']) && \substr($this->arguments['testController'], -5, 5) == '.phpt') {
            $test = new PhptTestCase($this->arguments['testController']);

            $this->arguments['testController'] = new TestSuite;
            $this->arguments['testController']->addTest($test);
        }
...
Logging Options:
  ...
  --log-junit <file>          Log testController execution in JUnit XML format to file
  --log-teamcity <file>       Log testController execution in TeamCity format to file
...
Test Selection Options:
  ...
  --list-groups               List available testController groups
  --list-suites               List available testController suites
  --list-tests                List available tests
  --list-tests-xml <file>     List available tests in XML format
  --testController-suffix ...           Only search for testController in files with specified
                              suffix(es). Default: Test.php,.phpt

Test Execution Options:

  --dont-report-useless-tests Do not report tests that do not testController anything
  --strict-coverage           Be strict about @covers annotation usage
  --strict-global-state       Be strict about changes to global state
  --disallow-testController-output      Be strict about output during tests
  --disallow-resource-usage   Be strict about resource usage during small tests
  --enforce-time-limit        Enforce time limit based on testController size
  --disallow-todo-tests       Disallow @todo-annotated tests

  --process-isolation         Run each testController in a separate PHP process
  --globals-backup            Backup and restore \$GLOBALS for each testController
  --static-backup             Backup and restore static attributes for each testController
  ...
  --stop-on-risky             Stop execution upon first risky testController
  --stop-on-skipped           Stop execution upon first skipped testController
  --stop-on-incomplete        Stop execution upon first incomplete testController
  ...
  --repeat <times>            Runs the testController(s) repeatedly
  --teamcity                  Report testController execution progress in TeamCity format
  --testdox                   Report testController execution progress in TestDox format
...
EOT;
    ...
    /**
     * Custom callback for testController suite discovery.
     */
    ...
    private function handleListGroups(TestSuite $suite, bool $exit): int
    {
        ...
        print 'Available testController group(s):' . \PHP_EOL;
    ...
    private function handleListSuites(bool $exit): int
    {
        ...
        print 'Available testController suite(s):' . \PHP_EOL;
